...
import { updateSession } from "./server/db/sessions";
...
export const refreshToken = async (refresh_token: string): Promise<Auth.AccessToken | null> => {
    ...
    try {
        const response = await axios.post(`${PUBLIC_TWITCH_OAUTH_BASE_URL}/token`, params);
        if (response.status !== 200 || !(response.data as Auth.AccessToken)?.access_token) return null;
        let token: Auth.AccessToken = response.data;
        let tokenExpiresAt = Date.now() + ((token?.expires_in || 0) * 1000);
        let tokenUser = await getUserFromToken(token.access_token);

        if(tokenUser) updateSession({access_token: token.access_token, refresh_token: token.refresh_token, expires_at: tokenExpiresAt, user_id: tokenUser.id})
