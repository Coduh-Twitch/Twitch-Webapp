<script lang="ts">
    ...
    import getItemSound from "$lib/assets/sound/zelda_getrareitem.flac";
    import completedSound from "$lib/assets/sound/zelda_orbfanfare.flac";
    ...
    async function fetchCounter(): Promise<DBCounter> {
        const res: ApiResponse<DBCounter> = await (await fetch(`/api/counters/spirit-orbs`)).json();
        return res.data;
    }
    ...
    onMount(async () => {
      ...
      setInterval(async () => {
        ...
        if(counter.count !== orbs) {
          if((counter.count !== 0) && counter.count > orbs && !isPlaying) display = true;
          orbs = counter.count;
          let isComplete = counter.count === maxOrbs;
          if(display) {
    ...
    let orbs: number = $state(0);
    let maxOrbs: number = $state(120);
            ...
            <div class="top {display ? "notif-in" : "notif-out"}">
                <div class="row">
                    <div class="row">
                        ...
                        <h1>{orbs === maxOrbs ? "ALL SHRINES COMPLETED!" : "SHRINE COMPLETED!"}</h1>
                    </div>
                    <div class="row">
                        <h3 class="objective">{counter.count} OF {maxOrbs}</h3>
                    </div>
                </div>
            </div>
            <!-- <div class="bottom">
