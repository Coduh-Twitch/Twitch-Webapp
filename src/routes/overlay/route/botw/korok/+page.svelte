<script lang="ts">
    ...
    async function fetchCounter(): Promise<DBCounter> {
        const res: ApiResponse<DBCounter> = await (await fetch(`/api/counters/korok`)).json();
        return res.data;
    }
    ...
    onMount(async () => {
      ...
      setInterval(async () => {
        ...
        if(counter.count !== koroks) {
          if((counter.count !== 0) && counter.count > koroks && !isPlaying) display = true;
          koroks = counter.count;
          let isComplete = counter.count === maxKoroks;
          if(display) {
            ...
            <div class="top {display ? "notif-in" : "notif-out"}">
                <div class="row">
                    <div class="row">
                        ...
                        <h1>{koroks === maxKoroks ? "ALL KOROKS SEEDS FOUND!" : "KOROK SEED FOUND!"}</h1>
                    </div>
                    <div class="row">
                        <h3 class="objective">{counter.count} OF {maxKoroks}</h3>
                    </div>
                </div>
            </div>
            <!-- <div class="bottom">
