<script lang="ts">
    ...
    import { Column, getUserData, Heading, setUserData, UserRoles } from "duckylib";
    ...
    onMount(async () => {
        ...
        let ai: ApiResponse<TwitchUser> = await (await fetch(`/api/users/@me`, {headers: {"x-requester-id": getUserData()?.id || "0"}})).json()
        console.log('ai', ai)
        

        if(!ai.data) return window.location.replace(`/login`);

        apiUser = {avatarUrl: ai.data.profile_image_url, id: ai.data.id, role: UserRoles.DEFAULT, username: ai.data.login}
