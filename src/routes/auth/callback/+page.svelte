<script lang="ts">
    ...
    import type { ApiResponse, TwitchUser } from "$lib/types";
    ...
    async function getUser(): Promise<Auth.User | null> {
        let r = await (
            await fetch(`/api/users/@me`, {
                headers: { "x-requester-id": getUserData()?.id || "0" },
            })
        ).json();
        return r.data || null;
    }
    ...
    onMount(async () => {
        if (page.url.searchParams.has("out") && browser) {
            setTimeout(() => {
                ...
                window.location.replace(
                    `/${page.url.searchParams.has("a") ? "" : "?a"}`,
                );
            }, 1e3);
        }
        let i = 0;

        let ai: ApiResponse<TwitchUser> = await (
            await fetch(`/api/users/@me`, {
                headers: { "x-requester-id": getUserData()?.id || "0" },
            })
        ).json();
        ...
        apiUser = {
            avatarUrl: ai.data.profile_image_url,
            id: ai.data.id,
            role:
                ai.data.login === PUBLIC_TWITCH_CHANNEL
                    ? UserRoles.ADMIN
                    : UserRoles.DEFAULT,
            username: ai.data.login,
        };

        if (!page.url.searchParams.has("out"))
            setInterval(async () => {
                i++;
                console.log(i);
                if (i > 4) {
                    if (apiUser) {
                        ...
                        apiUser = await (
                            await fetch(`/api/users/@me`, {
                                headers: {
                                    "x-requester-id": getUserData()?.id || "0",
                                },
                            })
                        ).json();
                        if (!apiUser && browser)
...
<Column>
    <Heading weight="bolder"
        >{apiUser !== null && apiUser?.username
            ? `${page.url.searchParams.has("out") ? "Seeya, " : "Hello, "}${apiUser.username}!`
            : "Redirecting..."}</Heading
    >
</Column>
