  ...
  if (!url.searchParams.has("out")) {
        ...
        if (refreshedToken && refreshedToken.access_token) {
          ...
          if (apiUser) {
            ...
            createDatabaseUser({
              ...
              id: apiUser.id,
              role:
                apiUser.login === PUBLIC_TWITCH_CHANNEL
                  ? UserRoles.ADMIN
                  : UserRoles.DEFAULT,
    ...
    let currentUser = await (
      await fetch(`/api/users/@me`, {
        headers: { "x-requester-id": apiUser?.id || "0" },
      })
    ).json();
