import { PRIVATE_MOD_ACCESS, PRIVATE_TWITCH_CHANNEL } from "$env/static/private";
...
export const GET = async ({url, locals, cookies}): Promise<Response> => {
    ...
    if(!code) {
        return redirect(308, `/auth/failure?from=${fromPath}&err=Authorization Cancelled`);
    } else {
        ...
        if(!token || !token.access_token) {
            ...
            return redirect(308, `/auth/failure?from=${fromPath}&err=Invalid Authorization Code`);
        } else {
            ...
            if(!tokenUser) {
                ...
                if((PRIVATE_MOD_ACCESS && PRIVATE_MOD_ACCESS === "enabled") && tokenUser.login !== PRIVATE_TWITCH_CHANNEL) {
                    if(token.scope && token.scope.includes("user:read:moderated_channels")) {
                        let moderatedChannels = await getModeratedChannels(tokenUser.id, token.access_token);
                        if(!moderatedChannels.find(c => c.broadcaster_login === PRIVATE_TWITCH_CHANNEL)) return redirect(308, `/auth/failure?from=${fromPath}&err=Not a Moderator`);
                    }
                } else {
                ...
                updateSession({user_id: userData.id, access_token: cookies.get("token-0") || "0", expires_at: tokenExpiresAt, refresh_token: cookies.get("token-r") || "r" || null})
