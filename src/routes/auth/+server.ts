import { PRIVATE_MOD_ACCESS } from "$env/static/private";
import { PUBLIC_TWITCH_CHANNEL } from "$env/static/public";
import { exchangeCodeForToken } from "$lib/auth";
import { updateSession } from "$lib/server/db/sessions.js";
import {
  createDatabaseUser,
  getDatabaseUser,
  updateDatabaseUser,
} from "$lib/server/db/users.js";
import { getModeratedChannels, getUserFromToken } from "$lib/twitch.js";
...
export const GET = async ({ url, locals, cookies }): Promise<Response> => {
  ...
  if (!code) {
    return redirect(
      308,
      `/auth/failure?from=${fromPath}&err=Authorization Cancelled`,
    );
  } else {
    ...
    if (!token || !token.access_token) {
      ...
      return redirect(
        308,
        `/auth/failure?from=${fromPath}&err=Invalid Authorization Code`,
      );
    } else {
      ...
      if (!tokenUser) {
        ...
        return redirect(
          308,
          `/auth/failure?from=${fromPath}&err=Token is Invalid`,
        );
      } else {
        if (
          PRIVATE_MOD_ACCESS &&
          PRIVATE_MOD_ACCESS === "enabled" &&
          tokenUser.login !== PUBLIC_TWITCH_CHANNEL
        ) {
          if (
            token.scope &&
            token.scope.includes("user:read:moderated_channels")
          ) {
            let moderatedChannels = await getModeratedChannels(
              tokenUser.id,
              token.access_token,
            );
            if (
              !moderatedChannels.find(
                (c) => c.broadcaster_login === PUBLIC_TWITCH_CHANNEL,
              )
            )
              return redirect(
                308,
                `/auth/failure?from=${fromPath}&err=Not a Moderator`,
              );
          }
        } else {
          if (tokenUser.login !== PUBLIC_TWITCH_CHANNEL)
            return redirect(
              308,
              `/auth/failure?from=${fromPath}&err=Invalid Channel`,
            );
        }
        let tokenExpiresAt = Date.now() + (token?.expires_in || 0) * 1000;
        let tokenExpiresInSeconds = token.expires_in;

        cookies.set(`token-0`, token.access_token, {
          path: "/",
        ...
        if (token.refresh_token)
          cookies.set(`token-r`, token.refresh_token, {
            path: "/",
            expires: new Date(tokenExpiresAt),
            maxAge: tokenExpiresInSeconds,
          });
        ...
        if (tokenUser.login === PUBLIC_TWITCH_CHANNEL) {
          ...
          let userData: Auth.User = {
            username: tokenUser.display_name,
            avatarUrl: tokenUser.profile_image_url,
            id: tokenUser.id,
            role: dbUser ? dbUser.role : UserRoles.DEFAULT,
          };
          ...
          updateSession({
            user_id: userData.id,
            access_token: cookies.get("token-0") || "0",
            expires_at: tokenExpiresAt,
            refresh_token: cookies.get("token-r") || "r" || null,
          });
        }
