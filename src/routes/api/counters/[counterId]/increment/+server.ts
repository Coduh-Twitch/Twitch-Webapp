...
export const POST = async ({fetch, params}) => {
  const counter = await (await fetch(`${PRIVATE_CHATBOT_APP_URL}/api/counters/${params.counterId}/increment`, {method: "POST", headers: {"key": PRIVATE_TWITCH_CLIENT_SECRET}})).json();
